...
@if (photoService.photoUrl()) {
  <img [src]="photoService.photoUrl()" />
}

<sl-button (click)="photoService.takePicture()">
...
// In component
<sl-button (click)="locationService.getCurrentLocation()">
...
@Component({
  ...
  template: `
    <sl-card class="event-detail">
      <h2>{{ event.name }}</h2>

      <!-- Show native actions on mobile -->
      @if (platform.isNative()) {
        <div class="mobile-actions">
          <sl-button (click)="shareEvent()"> <i class="fa-solid fa-share"></i> Share </sl-button>

          <sl-button (click)="navigateToLocation()">
            <i class="fa-solid fa-map"></i> Navigate
          </sl-button>

          @if (platform.supportsCamera()) {
            <sl-button (click)="takePhoto()"> <i class="fa-solid fa-camera"></i> Photo </sl-button>
          }
        </div>
      }
      ...
      @if (platform.isWeb) {
        <div class="web-actions">
          <sl-button (click)="copyLink()"> <i class="fa-solid fa-link"></i> Copy Link </sl-button>
        </div>
      }
    </sl-card>
  `,
