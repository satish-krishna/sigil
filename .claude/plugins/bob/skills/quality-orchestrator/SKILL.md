...
Read `state/history.json` (create if missing). Remove any finding where:
...
Read `state/backlog.json`. If it doesn't exist or is empty → output "No backlog. Run analysis first." and stop.

### Step 2: Pick Top N Findings

Default: **N = 3** (configurable).

Select the top N items from the queue where `status == "pending"`.

Skip items that:

- Have a matching entry in `state/history.json` with `status: "failed"` and `cooldownUntil` in the future
- Have a matching open PR (check `state/history.json` for `status: "pr-created"`)

If fewer than N items are eligible → take what's available. If zero → output "Backlog empty or all items in cooldown." and stop.
...
3. **Read result:**
   - `pr-created` → log to `state/history.json`, mark as done in backlog
...
Write updated `state/history.json`:
...
prompt: "Run the quality-orchestrator analysis pipeline (Pipeline 1). Invoke tech-debt-analyzer, enhancement-finder, and code-simplifier in parallel on the full repo. Merge findings, deduplicate, filter already-fixed items, prioritize, and write to state/backlog.json."
```
...
prompt: "Run the quality-orchestrator fix pipeline (Pipeline 2). Pick top 3 pending findings from state/backlog.json. For each, dispatch auto-fixer with TDD. Create PRs for successful fixes. Log all results to state/history.json."
```
